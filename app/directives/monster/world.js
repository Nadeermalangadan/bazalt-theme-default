define('directives/monster/world', [], function () {
    'use strict';
;
    var MonsterWorld = function (element) {

        var isMobile = (navigator.userAgent.toLowerCase().indexOf('android') != -1) || (navigator.userAgent.toLowerCase().indexOf('iphone') != -1);

        var worldRect = { x: 0, y: 0, width: 150, height:80};
        ...
        var mouseX = worldRect.width;
        var mouseY = worldRect.height;
        ...
        function loop() {
            ...
            for (i = 0, len = monsters.length; i < len; i++) {
                ...
                m.position.y = 18 + Math.max(Math.min(m.position.y, worldRect.height - (m.radius * 0.7)), (m.radius * 0.7));
                ...
                var color = context.createRadialGradient(m.position.x, m.position.y, 0, m.position.x, m.position.y, m.radius);
